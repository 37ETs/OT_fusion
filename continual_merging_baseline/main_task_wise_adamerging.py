...
from src.task_vectors import StateDict
...
args.model = model
args.save = source_root_path+'cache/checkpoints/task_vectors_checkpoints/'+model
args.logs_path = source_root_path + 'logs/' + model
pretrained_checkpoint = source_root_path +'cache/checkpoints/task_vectors_checkpoints/'+ model + '/zeroshot.pt'

str_time_ = time.strftime('%Y%m%d_%H%M%S', time.localtime(time.time()))
log = create_log_dir(args.logs_path, 'log_{}_Task_wise_AdaMerging.txt'.format(str_time_))
args.log = log

task_vectors = [TaskVector(pretrained_checkpoint, source_root_path+'cache/checkpoints/task_vectors_checkpoints/'+model+'/'+dataset_name+'/finetuned.pt') for dataset_name in exam_datasets]
if args.model == "ViT-B-32" or args.model == "ViT-B-16":
    task_vectors = [{k: v.cuda() for k, v in tv.vector.items()} for tv in task_vectors]
elif args.model == "ViT-L-14":
    temp = []
    for tv in task_vectors[:4]:
        temp.append({k: v.cuda(0, non_blocking=True) for k, v in tv.vector.items()})
    for tv in task_vectors[4:]:
        temp.append({k: v.cuda(1, non_blocking=True) for k, v in tv.vector.items()})
    task_vectors = temp
