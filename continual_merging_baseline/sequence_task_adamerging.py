...
class AdaMerging(torch.nn.Module):
    def __init__(self, paramslist, model, names, exam_datasets, init_task):
        ...
        prior = 0.3
        if init_task == 0:
            # init_task == 0 时，rlambdas 为 torch.ones(1, len(paramslist)-1) * prior
            rlambdas = torch.ones(1, len(paramslist)-1) * prior
        else:
            # init_task == 1 时，rlambdas 第一个元素为 1，后面元素为 prior
            rlambdas = torch.cat([torch.tensor([[1]]), torch.ones(1, len(paramslist) - 2) * prior], dim=1)
    ...
    def get_params(self):
        alph = self.lambdas()
        params = tuple(sum(tuple(pi * lambdasi for pi, lambdasi in zip(p, alph[0].cpu()))) for j, p in enumerate(zip(*self.paramslist[1:])))
        return params
...
for step in range(len(exam_datasets)):
    ...
    # if step == 0:
    #     seen_dataset.append(exam_datasets[step][0])
    # else:
    #     seen_dataset.append(exam_datasets[step-1][0])
    #     seen_dataset.append(exam_datasets[step][0]) 
...
for dataset_name in exam_datasets:
    image_encoder = adamerging_mtl_model.get_image_encoder()
    classification_head = adamerging_mtl_model.get_classification_head(dataset_name[0])
    #classification_head = get_classification_head(args, dataset_name[0])
    metrics = eval_single_dataset_preprocess_head(image_encoder, classification_head, dataset_name[0], args)
    Total_ACC += metrics['top1']
