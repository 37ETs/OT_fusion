...
from src.tasks.optimal_transport_mask import *
from src.tasks.salient_weight_mask import find_salient_weight_mask
...
class Program:
    ...
    def Optimal_Transport_Fusion(self):
        pretrained_model = self.pretrained_model
        task_vectors = self.task_vectors
        print("len(task_vectors):", len(task_vectors))
        for p in pretrained_model.parameters():
            p.detach_().requires_grad_(False)
        
        # 初始化第一个mask
        self.masks_pre_init = OptimalTransportMask(
            temperature=0.5,
            state_dict=task_vectors[0],
            init_value=0,
            draw_sample=True,
        )
        self.masks_post_init = OptimalTransportMask(
            temperature=0.5,
        ...
        #self.masks_pre = self.masks_pre_init
        #self.masks_post = self.masks_post_init
        
        # 逐步融合后续任务
        for i in range(0, len(task_vectors) - 1):
            self.ot_masks_pre, self.ot_masks_post = compute_ot_mask(state_dict_0=task_vectors[i], state_dict_1=task_vectors[i+1], masks_pre=self.masks_pre, masks_post=self.masks_post)
            fused_state_dict = fusion(state_dict_0=task_vectors[i], state_dict_1=task_vectors[i+1], ot_masks_pre=self.ot_masks_pre, ot_masks_post=self.ot_masks_post)
            self.fused_state_dict = fused_state_dict
            # 更新融合后的参数
            task_vectors[i+1] = fused_state_dict
            # 重新初始化mask
            self.masks_pre = self.masks_pre_init._draw_mask()
            self.masks_post = self.masks_post_init._draw_mask()
        ...
        torch.save(
            {
                "ot_masks_pre": self.ot_masks_pre,
                "ot_masks_post": self.ot_masks_post,
                "fused_state_dict": self.fused_state_dict,
            },
            self.ckpt_path,
        )
