...
def compute_sr_mask(
    ...
    for epoch in pbar:
        ...
        batch_pre = maybe_dictionarize(batch_pre)
        batch_post = maybe_dictionarize(batch_post)
        x_pre, y_pre = batch_pre["images"].to(device), batch_pre["labels"].to(device)
        x_post, y_post = batch_post["images"].to(device), batch_post["labels"].to(device)
        
        # 先将mask应用到融合模型权重(合并参数)
        model_merged.merge_weights()
        # 学生模型前向
        logits_student_pre = model_merged(x_pre)
        
        # 教师模型前向 (不计算梯度)
        with torch.no_grad():
            logits_teacher_pre = model_pre(x_pre)
            logits_teacher_post = model_post(x_post)
        ...
        loss_post = geomloss.SamplesLoss(loss="sinkhorn", p=2, blur=0.05)(logits_student_pre, logits_teacher_post)
        loss = 0.5*loss_pre + 0.5*loss_post
